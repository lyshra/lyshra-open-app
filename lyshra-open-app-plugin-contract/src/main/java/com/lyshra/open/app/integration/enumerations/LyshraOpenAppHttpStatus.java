...
/**
 * Represents an HTTP status code as defined in RFC 7231.
 * Code snippet has been copied from 'spring framework' to avoid spring dependency.
 */
public enum LyshraOpenAppHttpStatus {
	...
	private static final LyshraOpenAppHttpStatus[] VALUES;
	...
	LyshraOpenAppHttpStatus(int value, Series series, String reasonPhrase) {
		this.value = value;
		this.series = series;
	 ...
	 */
	public static LyshraOpenAppHttpStatus valueOf(int statusCode) {
		LyshraOpenAppHttpStatus status = resolve(statusCode);
		if (status == null) {
	 ...
	 */
	@Nullable
	public static LyshraOpenAppHttpStatus resolve(int statusCode) {
		// Use cached VALUES instead of values() to prevent array allocation.
		for (LyshraOpenAppHttpStatus status : VALUES) {
	 ...
	 * <p>Retrievable via {@link LyshraOpenAppHttpStatus#series()}.
	 */
	public enum Series {
		 ...
		 * @return the {@code Series} enum constant for the supplied {@code HttpStatus}
		 * @deprecated as of 5.3, in favor of invoking {@link LyshraOpenAppHttpStatus#series()} directly
		 */
		@Deprecated
		public static Series valueOf(LyshraOpenAppHttpStatus status) {
